...
//此代码是 python-Levenshtein 的端口，它是 Levenshtein distanceS 的高效（且经过混淆）实现
...
// EditDistance 计算两个字符串之间的编辑距离，对替换的权重与插入和删除的权重相同
func EditDistance(s1, s2 string) int {
	return LevEditDistance(s1, s2, 0)
}

// LevEditDistance 计算 2 个字符串之间的编辑距离。
// 如果 xcost 参数为零，则替换操作的权重为 1。
// 否则，所有编辑操作的权重均为 1。
