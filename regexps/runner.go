...
type runner struct {
	...
	// 回溯堆栈。操作码使用它来存储有关的数据
	// 他们匹配了什么以及回溯到哪里。每一个“帧”
	// 堆栈采用 [CodePosition Data1 Data2...] 的形式，其中
	// CodePosition 是当前操作码的位置
	// 数据值都是可选的。 CodePosition 可以为负数，并且
	// 这些值（也称为“back2”）由 BranchMark 操作码系列使用
	// 指示他们是在成功还是失败后回溯
	// 匹配。
	// 当我们回溯时，我们将 CodePosition 从堆栈中弹出，设置当前
	// 指向该代码位置的指令指针，并标记操作码
	// 带有回溯标志（“Back”）。每个操作码都知道如何
	// 处理自己的数据。
	runtrack    []int
	runtrackpos int

	// 该堆栈用于跟踪不同操作码之间的文本位置。
	// 例如，在 /(a*b)+/ 中，括号结果为 SetMark/CaptureMark
	// 一对。 SetMark 记录我们匹配 a*b 之前的文本位置。然后
	// CaptureMark 使用该位置来确定捕获开始的位置。
	// 压入该堆栈的操作码总是与其他操作码配对
	// 稍后会从中弹出值。一场成功的比赛应该意味着
	// 这个堆栈是空的。
	runstack    []int
	runstackpos int

	// 抓取堆栈用于跟踪捕获。每次都是一组
	// 有一个捕获，我们将其组号推送到 runcrawl 堆栈上。在
	// 在平衡匹配的情况下，我们将两个组都压入堆栈。
	runcrawl    []int
	runcrawlpos int
...
// run 搜索匹配项并可以从上一个匹配项继续
//
// fast 通常为 false，但也可以为 true，不返回匹配项，只是将其放入缓存中
// textstart 为 -1 表示从“开始”开始（取决于从右到左），否则为输入中的索引
// 输入是用于搜索正则表达式模式的字符串
