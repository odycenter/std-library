...
// 空间效率。
//
// RegexNodes 被构建成一棵树，通过 n.children 列表链接。
// 每个节点还有一个 n.parent 和 n.ichild 成员，表示
// 它的父级以及哪个子级 # 它位于其父级列表中。
//
// RegexNode 的类型与构造的类型一样多
// 正则表达式，例如“concatenate”、“alternate”、
// “一个”，“rep”，“组”。还有基本的节点类型
// 窥视孔优化，例如“onerep”、“notsetrep”等
//
// 因为 perl 5 允许向后扫描的“lookback”组，
// 每个节点也有一个“方向”。通常值为
// 布尔值 n.backward = false。
//
// 在解析过程中，顶级节点也被堆叠到解析中
// 堆栈（一堆树）。为此，我们有一个 n.next
// 指针。 [请注意，为了节省一些字节，我们可以重载
// 改为 n.parent 指针。]
//
// 在解析堆栈上，每棵树都有一个“角色” - 基本上，
// 解析器当前语法中的非终结符
// 分配给树。该代码存储在 n.role 中。
//
// 最后，一些不同类型的节点有数据。
// 两个整数（用于循环结构）存储在
// n.operands，一个对象（字符串或集合）
// 存储在n.data中
