...
// CharSet 结合了起始符文范围和表示一组字符的 unicode 类别
type CharSet struct {
	ranges     []singleRange
	categories []category
...
// CharIn 如果符文在我们的字符集中（范围或类别），则返回 true。
// 它处理否定和减去的子字符集。
...
// IsWordChar 根据 UTS#18 Unicode 正则表达式 (http://www.unicode.org/reports/tr18/)
// RL 1.4 简单单词边界 <word_character> 类包含所有字母
// 来自 Unicode 字符数据库的值，来自 UnicodeData.txt [UData]，加上 U+200C
// 零宽度非连接器和 U+200D 零宽度连接器。
...
// SingletonChar 将返回第一个范围中的字符而不进行验证。
// 它假设您已经检查了 IsSingleton 或 IsSingletonInverse，并且会因输入错误而发生恐慌
